...
- "R0_Cs_Ls_Cp_Lp_Rm" (backward-compatible alias)
...
def _compute_transport_current(
    ...
    # Cast to Python float so downstream circuit algebra stays in float64 even
    # when Gamma_i/Gamma_e are float32 plasma work arrays.
    return float((A * e / L) * integral_flux)


def _require(condition: bool, message: str) -> None:
    """Raise a user-facing error when a circuit parameter is invalid."""
    if not condition:
...
def update_circuit_no_external_component(
    ...
    _require(l > 0.0, "dielectric_plasma topology (Adamovic) requires l > 0.")
...
def update_circuit_R0_Cp(
    ...
    """
    _require(R0 > 0.0, "R0 must be > 0 for R0_Cp")
...
def update_circuit_R0_Cp_Rm(
    ...
    """
    # Basic checks
    _require(R0 > 0.0, "R0 must be > 0 for R0_Cp_Rm circuit.")
    _require(C_p > 0.0, "C_p must be > 0 for R0_Cp_Rm circuit.")
    _require(R_m > 0.0, "R_m must be > 0 for R0_Cp_Rm circuit.")
...
# ============================================================
# Case C2: R0 + Rm + load-side external capacitance
# ============================================================


def update_circuit_R0_Rm_Cext(
    V_app_func: Callable[[float], float],
    t: float,
    dt: float,
    V_gap_prev: float,
    ...
    """
    Update the circuit for an R0-Rm-plasma topology with load-side capacitance.

    Topology
    --------
        Vs(t) -- R0 -- V_n -- Rm -- V_load
                                      |
                                      +-- C_ext -- ground
                                      |
                                      +-- dielectric/plasma branch -- ground

    `C_ext` is an external load-side capacitance connected from the load
    terminal to ground. For dielectric-coated electrodes its voltage is the
    external terminal voltage V_load = alpha_d*V_gap + V_d, not the internal
    plasma-gap voltage alone. For bare electrodes, V_load = V_gap.

    This reduced topology treats V_n as an algebraic node because no source-side
    shunt capacitance is retained.

    Either R0 or R_m may be zero, which gives the useful single-ballast limits:

    - R0 > 0, R_m = 0: source-side ballast only, Vs -- R0 -- V_gap
    - R0 = 0, R_m > 0: measurement/load resistor only, Vs -- R_m -- V_gap

    The total series resistance R0 + R_m must remain positive.

    Returns
    -------
    V_n_new : float
        Updated algebraic node voltage between R0 and Rm [V].
    V_gap_new : float
        Updated plasma gap voltage [V].
    I_series : float
        Current through the series resistance path [A]. When R_m > 0, this is
        also the current through R_m.
    V_d_new : float
        Updated dielectric mapping voltage state [V].
    """
    _require(R0 >= 0.0, "R0 must be >= 0 for R0_Rm_Cext circuit.")
    _require(R_m >= 0.0, "R_m must be >= 0 for R0_Rm_Cext circuit.")
    _require(C_ext >= 0.0, "C_ext must be >= 0 for R0_Rm_Cext circuit.")
    R_total = R0 + R_m
    if R_total <= 0.0:
        raise ValueError("R0_Rm_Cext requires R0 + R_m > 0.")
    ...
    C_gap_eff = C_gap + alpha_d * C_ext
    if C_gap_eff <= 0.0:
        raise ZeroDivisionError("R0_Rm_Cext requires positive gap-node capacitance.")

    Vs = float(V_app_func(t))
    V_load_prev = alpha_d * V_gap_prev + V_d_prev

    # R0 and Rm are in series as seen by the load terminal. Use the previous
    # terminal voltage for the explicit Euler branch current.
    I_series_prev = (Vs - V_load_prev) / R_total
    dV_gap_dt = (I_series_prev - I_transport - C_ext * dV_d_dt) / C_gap_eff

    V_gap_new = V_gap_prev + dt * dV_gap_dt
    V_d_new = V_d_prev + dt * dV_d_dt
    V_load_new = alpha_d * V_gap_new + V_d_new
    V_n_new = (R_m * Vs + R0 * V_load_new) / R_total
    I_series = (Vs - V_load_new) / R_total

    return V_n_new, V_gap_new, I_series, V_d_new
...
def update_circuit_R0_Cs_Cp(
    ...
    """
    # Safety checks
    _require(R0 > 0.0, "R0 must be > 0 for R0_Cs_Cp circuit.")
    _require(C_s > 0.0, "C_s must be > 0 for R0_Cs_Cp circuit.")
    _require(C_p > 0.0, "C_p must be > 0 for R0_Cs_Cp circuit.")
...
def update_circuit_R0_Cs_Cp_Rm(
    ...
    """
    # Safety checks
    _require(R0 > 0.0, "R0 must be > 0 for R0_Cs_Cp_Rm circuit.")
    _require(C_s > 0.0, "C_s must be > 0 for R0_Cs_Cp_Rm circuit.")
    _require(C_p > 0.0, "C_p must be > 0 for R0_Cs_Cp_Rm circuit.")
    _require(R_m > 0.0, "R_m must be > 0 for R0_Cs_Cp_Rm circuit.")
...
def update_circuit_R0_Cs_Ls_Cp_Rm(
    ...
    """
    # Safety checks
    _require(R0 > 0.0, "R0 must be > 0 for R0_Cs_Ls_Cp_Rm circuit.")
    _require(C_s > 0.0, "C_s must be > 0 for R0_Cs_Ls_Cp_Rm circuit.")
    _require(L_s > 0.0, "L_s must be > 0 for R0_Cs_Ls_Cp_Rm circuit.")
    _require(C_p > 0.0, "C_p must be > 0 for R0_Cs_Ls_Cp_Rm circuit.")
    _require(R_m > 0.0, "R_m must be > 0 for R0_Cs_Ls_Cp_Rm circuit.")
...
def update_circuit_R0_Cs_Ls_Cp(
    ...
    """
    # Safety checks
    _require(R0 > 0.0, "R0 must be > 0 for R0_Cs_Ls_Cp circuit.")
    _require(C_s > 0.0, "C_s must be > 0 for R0_Cs_Ls_Cp circuit.")
    _require(L_s > 0.0, "L_s must be > 0 for R0_Cs_Ls_Cp circuit.")
    _require(C_p > 0.0, "C_p must be > 0 for R0_Cs_Ls_Cp circuit.")
...
def update_circuit_R0_Cs_Ls_Cp_Lp(
    ...
    """
    # Safety checks
    _require(R0 > 0.0, "R0 must be > 0 for R0_Cs_Ls_Cp_Lp circuit.")
    _require(C_s > 0.0, "C_s must be > 0 for R0_Cs_Ls_Cp_Lp circuit.")
    _require(L_s > 0.0, "L_s must be > 0 for R0_Cs_Ls_Cp_Lp circuit.")
    _require(C_p > 0.0, "C_p must be > 0 for R0_Cs_Ls_Cp_Lp circuit.")
    _require(L_p > 0.0, "L_p must be > 0 for R0_Cs_Ls_Cp_Lp circuit.")
...
def update_circuit_R0_Cs_Ls_Cp_Lp_Rm_Cext(
    V_app_func: Callable[[float], float],
    t: float,
    dt: float,
    V_n_prev: float,
    ...
    R0, C_s, L_s, C_p, L_p, R_m : float
        Circuit component values (SI units).
    C_ext : float, optional
        External load-side capacitance [F] connected from V_load to ground.
        For dielectric-coated electrodes, V_load = alpha_d*V_gap + V_d.
    ...
    V_gap_new : float
        Updated plasma gap voltage [V].
    I_load : float
        Current through Rm into the load side [A]. For C_ext = 0 this reduces
        to the plasma-branch current of the C_ext-free topology.
    V_Cs_new : float
        Updated Cs voltage [V].
    I_s_new : float
        Updated series branch current [A].
    ...
    """
    # Safety checks
    _require(R0 > 0.0, "R0 must be > 0 for R0_Cs_Ls_Cp_Lp_Rm_Cext circuit.")
    _require(C_s > 0.0, "C_s must be > 0 for R0_Cs_Ls_Cp_Lp_Rm_Cext circuit.")
    _require(L_s > 0.0, "L_s must be > 0 for R0_Cs_Ls_Cp_Lp_Rm_Cext circuit.")
    _require(C_p > 0.0, "C_p must be > 0 for R0_Cs_Ls_Cp_Lp_Rm_Cext circuit.")
    _require(L_p > 0.0, "L_p must be > 0 for R0_Cs_Ls_Cp_Lp_Rm_Cext circuit.")
    _require(R_m > 0.0, "R_m must be > 0 for R0_Cs_Ls_Cp_Lp_Rm_Cext circuit.")
    _require(C_ext >= 0.0, "C_ext must be >= 0 for R0_Cs_Ls_Cp_Lp_Rm_Cext circuit.")
    ...
    # 6) Gap-voltage dynamics from branch-current balance:
    #    I_Rm = I_transport + C_gap*dV_gap/dt + C_ext*dV_load/dt
    #    V_load = alpha_d*V_gap + V_d
    V_load_prev = alpha_d * V_gap_prev + V_d_prev
    I_Rm_prev = (V_n_prev - V_load_prev) / R_m
    C_gap_eff = C_gap + alpha_d * C_ext
    if C_gap_eff <= 0.0:
        raise ZeroDivisionError("R0_Cs_Ls_Cp_Lp_Rm_Cext requires positive load capacitance.")

    dV_gap_dt = (
        I_Rm_prev - I_transport - C_ext * dV_d_dt
    ) / C_gap_eff

    # 7) Node KCL with Cp, Lp, and plasma branch:
    #    I_s = I_Cp + I_Lp + I_Rm
    dV_n_dt = (
        I_s_prev - I_Lp_prev - I_Rm_prev
    ) / C_p
    ...
    # Current through Rm into the load side. For C_ext = 0 this reduces to the
    # plasma-branch current of the C_ext-free topology.
    dV_load_dt = alpha_d * dV_gap_dt + dV_d_dt
    I_load = I_transport + C_gap * dV_gap_dt + C_ext * dV_load_dt

    return V_n_new, V_gap_new, I_load, V_Cs_new, I_s_new, I_Lp_new, V_d_new
...
def step_circuit(
    ...
    C_ext : float, optional
        External load-side capacitance [F] connected from the load terminal to
        ground for topologies that support it, currently R0_Rm_Cext and
        R0_Cs_Ls_Cp_Lp_Rm_Cext. The old name R0_Cs_Ls_Cp_Lp_Rm remains a
        backward-compatible alias. For dielectric-coated electrodes, the
        load-terminal voltage is alpha_d*V_gap + V_d. This capacitance excludes
        the geometric gas-gap capacitance eps0*A/L, which is included internally.
    ...
    V_gap_new : float
        Updated plasma gap voltage [V].
    I_discharge : float
        Discharge/external current reported by the selected topology [A].
        For plasma-branch topologies this is the conduction plus geometric
        displacement current. For R0_Rm_Cext, this is the external series
        current through the resistance path, including load-side C_ext
        charging when C_ext > 0.
    ...
    # ------------------------------------------------------------
    # 1) R0–Cs–Ls–Cp–Lp–Rm–Cext + dielectric-aware load branch
    # ------------------------------------------------------------
    if circuit_type in ("R0_Cs_Ls_Cp_Lp_Rm_Cext", "R0_Cs_Ls_Cp_Lp_Rm"):
        if R_m <= 0.0:
            ...
            if any(v is None for v in (V_n_prev, V_Cs_prev, I_s_prev, I_Lp_prev)):
                raise ValueError(
                    "R0_Cs_Ls_Cp_Lp_Rm_Cext requires V_n_prev, V_Cs_prev, I_s_prev, I_Lp_prev."
                )

            if V_d_prev is None:
                raise ValueError("Dielectric mapping requires V_d_prev.")

            V_n_new, V_gap_new, I_load, V_Cs_new, I_s_new, I_Lp_new, V_d_new = (
                update_circuit_R0_Cs_Ls_Cp_Lp_Rm_Cext(
                    V_app_func=V_app_func,
                    t=t,
                    dt=dt,
                    V_n_prev=V_n_prev,
            ...
            return V_gap_new, I_load, V_d_new, V_n_new, V_Cs_new, I_s_new, I_Lp_new
    ...
    # ------------------------------------------------------------
    # 6) R0–Rm + external load-side capacitance
    # ------------------------------------------------------------
    if circuit_type == "R0_Rm_Cext":
        if V_d_prev is None:
            raise ValueError("R0_Rm_Cext requires V_d_prev.")

        V_n_new, V_gap_new, I_Rm, V_d_new = update_circuit_R0_Rm_Cext(
            V_app_func=V_app_func,
            t=t,
            dt=dt,
            V_gap_prev=V_gap_prev,
        ...
        return V_gap_new, I_Rm, V_d_new, V_n_new, None, None, None

    # ------------------------------------------------------------
    # 7) R0–Cp–Rm + dielectric-aware plasma branch
    # ------------------------------------------------------------
    ...
    # ------------------------------------------------------------
    # 8) Simple R0–Cp (+ optional dielectric) topology
    # ------------------------------------------------------------
    ...
    # ------------------------------------------------------------
    # 9) Plasma-only Adamovic topology (no explicit external elements)
    # ------------------------------------------------------------
    ...
    # ------------------------------------------------------------
    # 10) Fallback/auto-detect based on parameter values
    # ------------------------------------------------------------
