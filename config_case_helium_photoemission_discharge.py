...
Standalone complete configuration for the helium photoemission case.
...
The numerical defaults define a helium photoemission transient with a reduced
source-side circuit:
...
The case uses a 231 V applied step, cathode quantum photoemission, BOLSIG+
helium electron data, LXCat 4He+/He ion mobility, and Einstein-relation ion
diffusion. Sweep scripts should override individual fields of
``SimulationConfig`` rather than putting loops inside this config file.
...
# `CONFIG_MODULE = "config_case_helium_photoemission_discharge"` in
...
#
# The defaults define the helium photoemission table-transport example: a 231 V
# step drive, cathode quantum photoemission, BOLSIG+ helium electron transport,
# LXCat 4He+/He mobility, and Einstein-relation ion diffusion.
...
class RunConfig:
    ...
    run_name: str = "helium_photoemission_smoke_test"
...
class PlasmaConfig:
    ...
    electron_kinetics_model: ElectronKineticsModel = "local_field_approximation"
    ...
    ion_kinetics_model: IonKineticsModel = "local_field_ion_kinetics"
...
class TownsendCoefficientConfig:
    ...
    townsend_alpha_swarm_data_path: str = "he_swarm_output_full_EoverN.dat"
...
class IonizationFrequencySourceConfig:
    ...
    # electron_swarm_data). If None, the default electron table is reused:
    # - E/N axis -> local_field_approximation.electron_swarm_data_path
    ionization_frequency_swarm_data_path: str = "he_swarm_output_full_EoverN.dat"
...
class LocalFieldApproximationConfig:
    ...
    electron_swarm_data_path: str = "he_swarm_output_full_EoverN.dat"
...
class IonTransportConfig:
    ...
    mobility_source_mode: IonMobilitySourceMode = "swarm_data_table_interpolation"
    ...
    #   mobility profile
    diffusion_source_mode: IonDiffusionSourceMode = "einstein_relation"
    ...
    mobility_table_path: str | None = "lxcat_57511c29d0dafef749d3_reduced_mobility.csv"
...
# **Default case**
#
# The default enables cathode quantum-pulse photoemission only. The laser energy
# is chosen so the model computes the represented emitted charge for the
# baseline PASCHEN-1D photoemission example. No hidden target-charge
...
class EmissionConfig:
    ...
    shared_laser_U_J: float = 24.751695300e-6
    ...
    shared_emission_eps_points: int = 100
    ...
    anode_laser_t0: float = 1e-6
    anode_emission_lambda_m: float = 230e-9
    anode_laser_U_J: float = 24.751695300e-6
    ...
    anode_emission_eps_points: int = 100
    ...
    cathode_laser_t0: float = 1e-6
    cathode_emission_lambda_m: float = 230e-9
    cathode_laser_U_J: float = 24.751695300e-6
    ...
    cathode_emission_eps_points: int = 100
