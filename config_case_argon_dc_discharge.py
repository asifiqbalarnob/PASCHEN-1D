...
Complete PASCHEN-1D configuration schema for an argon DC discharge/glow case.

This module keeps the same full user-facing layout as the generic
configuration file, but its defaults are tuned for an argon DC discharge/glow
reference example. The circuit is represented with the
new reduced source/load topology:
...
For this DC-glow default, R0 = 0 and C_ext = 0. The 1 MOhm external ballast
resistor is represented as R_m, i.e. a measurement/ballast resistor directly in
series with the plasma branch. This is equivalent to an R0_Cp setup with
C_p = 0 for the bare DC circuit, while using the clearer
public source/load naming.
...
class CircuitConfig:
    ...
    # Source-side effective resistance [ohm].
    R0: float = 0.0
