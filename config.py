...
Guided generic PASCHEN-1D configuration module.
...
The default case corresponds to a 231 V applied step, a cathode quantum
photoemission pulse producing about 210 pC represented emitted charge, and an
external load-side stray capacitance of 16 pF.  Sweep scripts should override
...
#
# This file is the default configuration module loaded when a user selects
# `CONFIG_MODULE = "config"` in `run_paschen_1d.ipynb`.
#
...
# 5. For reusable cases, copy this file or one of the `config_case_*.py`
#    examples and edit the defaults there.
#
...
#
# The defaults reproduce the public argon photoemission-discharge example:
# a 231 V step drive, cathode-side quantum photoemission, an effective
# source-side sag resistance `R0 = 0.40 MOhm`, a measurement/load resistor
# `R_m = 1 MOhm`, and an external load-side stray capacitance
# `C_ext = 16 pF`.
...
#   `dt = T_total / Nt`, where `Nt` is set in `NumericsConfig`.
...
class RunConfig:
    ...
    run_name: str = "argon_photoemission_induced_discharge"
...
class CircuitConfig:
    ...
    #
    #   "R0_Cs_Ls_Cp_Lp_Rm"
    #       Backward-compatible alias for "R0_Cs_Ls_Cp_Lp_Rm_Cext" with C_ext=0.
    #
    #   "R0_Cs_Ls_Cp_Lp_Rm_Cext"
...
# - `warn_on_config_mismatch` prints non-fatal consistency warnings when a
#   selected mode and related fields appear inconsistent.
