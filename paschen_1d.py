...
def _print_run_config_summary(cfg: SimulationConfig, dt: float, dx: float) -> None:
    ...
    print(
        ...
        f"L_s={cfg.circuit.L_s:.6g}, C_p={cfg.circuit.C_p:.6g}, L_p={cfg.circuit.L_p:.6g}, R_m={cfg.circuit.R_m:.6g}"
    )
    print(
        "numerics: "
    ...
    print(
        ...
        f"to_circuit={cfg.emission.enable_emission_in_circuit_current}"
    )
    print(
        "output: "
...
def _print_config_warnings(cfg: SimulationConfig) -> None:
    ...
    if (not cfg.emission.enable_external_emission) and cfg.emission.enable_emission_in_circuit_current:
...
def run_configured_diagnostics(
    ...
    if temporal_cfg.enabled:
        ...
        if state.picard_iterations is not None:
            temporal_values["picard_iterations"] = np.asarray(
                state.picard_iterations, dtype=np.float64
            )
        if state.adaptive_substeps is not None:
            temporal_values["adaptive_substeps"] = np.asarray(
                state.adaptive_substeps, dtype=np.float64
            )
        if state.adaptive_dt_sub is not None:
            temporal_values["adaptive_dt_sub"] = np.asarray(
                state.adaptive_dt_sub, dtype=np.float64
            )
        if state.adaptive_cfl_est is not None:
            temporal_values["adaptive_cfl_est"] = np.asarray(
                state.adaptive_cfl_est, dtype=np.float64
            )
        ...
        if particle_inventory_required:
            nsave = int((cfg.numerics.Nt - 1) // cfg.output.save_every + 1)
            saved_indices = np.arange(nsave, dtype=np.int64) * cfg.output.save_every
            saved_indices = np.minimum(saved_indices, cfg.numerics.Nt - 1)
            saved_times_inventory = state.time[saved_indices]

            outdir_inventory = Path(cfg.run.run_name)
            ne_path = outdir_inventory / "ne_sampled_mm.dat"
            ni_path = outdir_inventory / "ni_sampled_mm.dat"
            if ne_path.exists() and ni_path.exists():
                ne_sampled = np.memmap(ne_path, mode="r", dtype=np.float32, shape=(nsave, cfg.numerics.Nx))
                ni_sampled = np.memmap(ni_path, mode="r", dtype=np.float32, shape=(nsave, cfg.numerics.Nx))
                N_e = cfg.geometry.A * np.trapz(np.asarray(ne_sampled, dtype=np.float64), x=state.x, axis=1)
                N_i = cfg.geometry.A * np.trapz(np.asarray(ni_sampled, dtype=np.float64), x=state.x, axis=1)
    ...
    sampled_paths = {
        ...
        "S_ion": outdir / "S_ion_sampled_mm.dat",
        "S": outdir / "S_sampled_mm.dat",
        "mu_e": outdir / "mu_e_sampled_mm.dat",
        "D_e": outdir / "D_e_sampled_mm.dat",
    }
    ...
    if state.mu_e_final is not None:
        final_profiles["mu_e"] = state.mu_e_final
    if state.D_e_final is not None:
        final_profiles["D_e"] = state.D_e_final
    ...
    for quantity in spatial_cfg.quantities:
        ...
        if path is not None and path.exists():
            sampled_arrays[quantity] = np.memmap(
                path, mode="r", dtype=np.float32, shape=(nsave, cfg.numerics.Nx)
            )
    ...
    if averaged_cfg.mode == "time_window":
        ...
        if cfg.waveform.waveform_type != "rf" or cfg.waveform.f_rf <= 0.0:
            print(
        ...
        if n_cycles <= 0:
            ...
            return
        rf_period = 1.0 / float(cfg.waveform.f_rf)
...
def run_simulation(cfg: SimulationConfig) -> SimulationState:
    ...
    # Legacy placeholders for inactive LEA integral closure.
    ...
    for n_idx in tqdm(range(Nt - 1), mininterval=2, desc="Time stepping"):
        ...
        emission_to_circuit = (emission_model is not None) and cfg.emission.enable_emission_in_circuit_current
        ...
        for sub_idx in range(n_sub):
            ...
            if cfg.boundary.enable_volume_sources:
                ionization_source = (
                    S_ion_row if cfg.boundary.enable_ionization_source else 0.0
                )
                recombination_sink = (
...
if __name__ == "__main__":
    ...
    cfg.waveform.V_peak = 200.0
    cfg.plasma_state.p_Torr = 1.0
    cfg.plasma_state.gas = "argon"
    cfg.run.run_name = "argon_pd1torrcm_vapp200V_step"
