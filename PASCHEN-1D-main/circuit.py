...
def step_circuit(
    ...
    C_ext : float, optional
        ...
        R0_Cs_Ls_Cp_Lp_Rm_Cext. For dielectric-coated electrodes, the
    ...
    # ------------------------------------------------------------
    if circuit_type == "R0_Cs_Ls_Cp_Lp_Rm_Cext":
        if any(v is None for v in (V_n_prev, V_Cs_prev, I_s_prev, I_Lp_prev)):
            raise ValueError(
    ...
    if circuit_type == "R0_Cs_Ls_Cp_Rm":
        _require(R_m > 0.0, "R0_Cs_Ls_Cp_Rm requires R_m > 0.")
    ...
    if circuit_type == "R0_Cs_Cp_Rm":
        _require(R_m > 0.0, "R0_Cs_Cp_Rm requires R_m > 0.")
        if V_n_prev is None or V_Cs_prev is None:
            raise ValueError(
    ...
    if circuit_type == "R0_Cp_Rm":
        _require(R_m > 0.0, "R0_Cp_Rm requires R_m > 0.")
        if V_n_prev is None:
