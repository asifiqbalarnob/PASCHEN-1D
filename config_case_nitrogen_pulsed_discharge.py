...
Complete PASCHEN-1D configuration schema for a nitrogen pulsed-discharge case.

This module keeps the same full user-facing layout as the generic
configuration file, but its defaults are tuned for a nitrogen pulsed
dielectric/plasma reference example. The default circuit
is the built-in dielectric/plasma branch with no additional lumped external
circuit elements.
